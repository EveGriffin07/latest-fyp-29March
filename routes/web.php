...
use App\Http\Controllers\ManagerInterviewController;
...
// Manual Verification (No Auth Required)
Route::get('/email/verify/{id}/{hash}', function ($id, $hash, Request $request) {
    $user = User::findOrFail($id);

    // Verify the hash is correct for security
...
Route::prefix('admin')->middleware(['auth', 'role:admin,administrator,hr,manager'])->group(function () {
    ...
    Route::post('/training/{id}/approve-request', [TrainingController::class, 'approveRequest'])->name('admin.training.approve_request');
    Route::post('/training/{id}/reject-request', [TrainingController::class, 'rejectRequest'])->name('admin.training.reject_request');
...
Route::prefix('applicant')->middleware(['auth', 'role:applicant', 'verified'])->group(function () {
    ...
    Route::post('/profile/certification', [ApplicantJobController::class, 'storeCertification'])->name('applicant.certification.store');
    Route::delete('/profile/certification/{id}', [ApplicantJobController::class, 'deleteCertification'])->name('applicant.certification.destroy');
});
...
|--------------------------------------------------------------------------
*/
Route::prefix('manager/interviews')->middleware(['auth', 'role:supervisor,manager'])->group(function () {
    Route::get('/', [ManagerInterviewController::class, 'index'])->name('manager.interviews.index');
    Route::post('/{id}/accept', [ManagerInterviewController::class, 'accept'])->name('manager.interviews.accept');
    Route::post('/{id}/reject', [ManagerInterviewController::class, 'reject'])->name('manager.interviews.reject');
    Route::post('/{id}/evaluate', [ManagerInterviewController::class, 'evaluate'])->name('manager.interviews.evaluate');
});
...
Route::middleware(['auth', 'role:employee,supervisor'])->group(function () {
    ...
    Route::post('/employee/training/{id}/enroll', [EmployeeTrainingController::class, 'selfEnroll'])->name('employee.training.self_enroll');
...
Route::prefix('supervisor')->middleware(['auth', 'role:supervisor'])->group(function () {
    ...
    // Supervisor Training Requests
    Route::post('/training/request', [TrainingController::class, 'storeSupervisorRequest'])->name('supervisor.training.request');
});