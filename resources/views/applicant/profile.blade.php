    ...
    <div class="profile-content" style="flex: 3; background: #fff; padding: 40px; border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.05);">
        ...
        <div class="card-section" style="margin-bottom: 35px;">
            ...
            <div class="form-row" style="display: flex; gap: 20px;">
                ...
                <div class="form-group autocomplete-wrapper" style="width: 100%;">
                    <label style="display: block; margin-bottom: 5px; font-weight: 500; font-size: 14px; color: #4b5563;">Department of Interest</label>
                    {{-- Note: Keeping the name 'industry_interest' so your database saves it correctly --}}
                    <input type="text" name="industry_interest" id="industryInput" class="autocomplete-input letters-only" value="{{ old('industry_interest', $profile->industry_interest ?? '') }}" placeholder="Type or select a department..." form="mainProfileForm" autocomplete="off">
                    
                    <ul class="autocomplete-list" id="industryList">
                        {{-- Dynamically loop through the departments from the database --}}
                        @foreach($departments as $department)
                            <li class="autocomplete-item">{{ $department->department_name }}</li> 
                        @endforeach
                    </ul>
                </div>
        ...
        <div class="card-section" style="margin-bottom: 35px;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
                ...
                <div style="display: flex; gap: 15px; align-items: center;">
                    <button type="button" onclick="toggleFullSection('experience-list', this)" style="background: none; border: none; color: #4b5563; font-weight: 500; cursor: pointer; font-size: 14px;">
                        <i class="fa-solid fa-eye"></i> View
                    </button>
                    <button type="button" onclick="document.getElementById('experienceModal').style.display='block'" style="background: none; border: none; color: #2563eb; font-weight: 600; cursor: pointer; font-size: 15px;">
                        + Add
                    </button>
                </div>
            </div>
            
            @php $experiences = $profile->experiences()->get(); @endphp
            
            {{-- THE WRAPPER (Hidden by default) --}}
            <div id="experience-list" style="display: none;">
        ...
        <div class="card-section" style="margin-bottom: 35px;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
                ...
                {{-- Button Group --}}
                <div style="display: flex; gap: 15px; align-items: center;">
                    <button type="button" onclick="toggleFullSection('education-list', this)" style="background: none; border: none; color: #4b5563; font-weight: 500; cursor: pointer; font-size: 14px;">
                        <i class="fa-solid fa-eye"></i> View
                    </button>
                    <button type="button" onclick="document.getElementById('educationModal').style.display='block'" style="background: none; border: none; color: #2563eb; font-weight: 600; cursor: pointer; font-size: 15px;">
                        + Add
                    </button>
                </div>
            </div>
            
            @php $educations = $profile->educations()->get(); @endphp
            
            {{-- THE WRAPPER (Hidden by default) --}}
            <div id="education-list" style="display: none;">
        ...
        <div class="card-section" style="margin-bottom: 35px;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
                ...
                {{-- Button Group --}}
                <div style="display: flex; gap: 15px; align-items: center;">
                    <button type="button" onclick="toggleFullSection('skill-list', this)" style="background: none; border: none; color: #4b5563; font-weight: 500; cursor: pointer; font-size: 14px;">
                        <i class="fa-solid fa-eye"></i> View
                    </button>
                    <button type="button" onclick="document.getElementById('skillModal').style.display='block'" style="background: none; border: none; color: #2563eb; font-weight: 600; cursor: pointer; font-size: 15px;">
                        + Add
                    </button>
                </div>
            </div>
            
            @php $skills = $profile->skills()->get(); @endphp
            
            {{-- THE WRAPPER (Hidden by default) --}}
            <div id="skill-list" style="display: none;">
        ...
        <div class="card-section" style="margin-bottom: 35px;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
                ...
                {{-- Button Group --}}
                <div style="display: flex; gap: 15px; align-items: center;">
                    <button type="button" onclick="toggleFullSection('language-list', this)" style="background: none; border: none; color: #4b5563; font-weight: 500; cursor: pointer; font-size: 14px;">
                        <i class="fa-solid fa-eye"></i> View
                    </button>
                    <button type="button" onclick="document.getElementById('languageModal').style.display='block'" style="background: none; border: none; color: #2563eb; font-weight: 600; cursor: pointer; font-size: 15px;">
                        + Add
                    </button>
                </div>
            </div>
            
            @php $languages = $profile->languages()->get(); @endphp

            {{-- THE WRAPPER (Hidden by default) --}}
            <div id="language-list" style="display: none;">
        ...
        <div class="card-section" style="margin-bottom: 35px;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 40px; margin-bottom: 15px;">
                <h3 style="font-size: 16px; color: #1f2937; font-weight: 600;">
                    <i class="fa-solid fa-certificate" style="color: #2563eb; margin-right: 8px;"></i> 
                    Licenses & Certifications
                </h3>
                
                {{-- Button Group --}}
                <div style="display: flex; gap: 15px; align-items: center;">
                    <button type="button" onclick="toggleFullSection('certification-list', this)" style="background: none; border: none; color: #4b5563; font-weight: 500; cursor: pointer; font-size: 14px;">
                        <i class="fa-solid fa-eye"></i> View
                    </button>
                    <button type="button" onclick="document.getElementById('certificationModal').style.display='block'" style="background: none; border: none; color: #2563eb; font-weight: 600; cursor: pointer; font-size: 15px;">
                        + Add
                    </button>
                </div>
            </div>
            
            @php $certifications = $profile->certifications()->get(); @endphp

            {{-- THE WRAPPER (Hidden by default) --}}
            <div id="certification-list" style="display: none;">
                <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 15px;">
                    @foreach($certifications as $cert)
                    <div style="border: 1px solid #e5e7eb; border-radius: 8px; padding: 15px; width: calc(50% - 8px); display: flex; justify-content: space-between; align-items: center;">
                        <div>
                            <h4 style="font-size: 15px; font-weight: 600; color: #1f2937;">{{ $cert->cert_name }}</h4>
                            <p style="font-size: 13px; color: #6b7280; margin-top: 2px;">{{ $cert->issued_by }}</p>
                        </div>
                        <form action="{{ route('applicant.certification.destroy', $cert->id) }}" method="POST">
                            @csrf @method('DELETE')
                            <button type="submit" style="background: none; border: none; color: #9ca3af; cursor: pointer;" onclick="return confirm('Delete this certification?')"><i class="fa-solid fa-trash"></i></button>
                        </form>
                    </div>
                    @endforeach
                </div>
                ...
                @if($certifications->count() == 0)
                    <p style="color: #9ca3af; font-size: 14px; font-style: italic; margin-top: -5px;">No certifications added yet.</p>
                @endif
            </div>
        </div>

        <div id="certificationModal" class="hrms-modal">
    <div class="hrms-modal-content">
        <span class="hrms-close" onclick="document.getElementById('certificationModal').style.display='none'">&times;</span>
        <h3 style="margin-bottom: 20px; font-size: 20px; color: #111827;">Add Certification</h3>
        <form action="{{ route('applicant.certification.store') }}" method="POST">
            @csrf
            <div style="margin-bottom: 15px;">
                <label class="modal-label">Certification / License Name</label>
                <input type="text" name="cert_name" class="modal-input" required placeholder="e.g. AWS Certified Developer">
            </div>
            <div style="margin-bottom: 20px;">
                <label class="modal-label">Issuing Organization</label>
                <input type="text" name="issued_by" class="modal-input" required placeholder="e.g. Amazon Web Services">
            </div>
            <div style="margin-bottom: 20px;">
                <label class="modal-label">Issue Date (Optional)</label>
                <input type="date" name="issue_date" class="modal-input">
            </div>
            <button type="submit" style="background: #2563eb; color: #fff; padding: 12px; border: none; border-radius: 6px; cursor: pointer; width: 100%; font-weight: 600;">Save Certification</button>
        </form>
    </div>
</div>
...
<script>
    ...
    // ==========================================================
    // 6. EDUCATION DATE VALIDATION (Chronology & Duration)
    // ==========================================================
    const educationForm = document.querySelector('#educationModal form');
    if (educationForm) {
        educationForm.addEventListener('submit', function(e) {
            const startDateStr = this.querySelector('input[name="start_date"]').value;
            const endDateStr = this.querySelector('input[name="end_date"]').value;
            const degreeTitle = this.querySelector('input[name="degree_title"]').value.toLowerCase();
            ...
            if (startDateStr && endDateStr) {
                ...
                // Check 2: Degree Duration (3-4 years logic)
                if (degreeTitle.includes('degree') || degreeTitle.includes('bachelor')) {
                    // Calculate difference in years
                    const diffTime = Math.abs(end - start);
                    const diffYears = diffTime / (1000 * 60 * 60 * 24 * 365.25);

                    // Standard degree takes ~3-4 years. We give a buffer (e.g., 2.5 to 5 years)
                    if (diffYears < 2.5 || diffYears > 5.5) {
                        const confirmProceed = confirm(
                            `Validation Warning:\nA typical degree takes 3 to 4 years, but your selected dates result in a duration of ${diffYears.toFixed(1)} years.\n\nAre you sure these dates are correct?\nClick 'Cancel' to fix them, or 'OK' to proceed.`
                        );
                        
                        if (!confirmProceed) {
                            e.preventDefault(); // Stop form submission if they click Cancel
                        }
                    }
                }
            }
    ...
    // ==========================================================
    // ==========================================================
    // 7. TOGGLE FULL SECTIONS (Accordion Style)
    // ==========================================================
    function toggleFullSection(sectionId, btn) {
        const section = document.getElementById(sectionId);
        
        if (section.style.display === 'none') {
            section.style.display = 'block'; // Or 'flex' depending on your inner layout
