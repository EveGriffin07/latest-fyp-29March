    ...
    <main>
            ...
            <div class="grid-info">
                ...
                <div class="info-box" style="align-items: center; text-align: center;">
                    ...
                    @php
                        ...
                        $isActiveToday = $today->betweenIncluded($start, $end);
                    @endphp

                    @if($program->qr_token && $isActiveToday)
                        ...
                        <strong style="color: #10b981; font-size: 12px; margin-top: 10px;">Active Today</strong>
                    @elseif($program->qr_token && $today->lt($start))
                        <strong style="color: #94a3b8; margin-top: 20px;">QR reveals on {{ $start->format('d M') }}</strong>
                    @elseif($program->qr_token && $today->gt($end))
                        <strong style="color: #dc2626; margin-top: 20px;">Training Ended</strong>
                    @else
        ...
        <div style="background:#fff; border-radius:12px; border:1px solid #e2e8f0; overflow: hidden; box-shadow: 0 4px 6px rgba(0,0,0,0.02);">
            <table class="hr-table">
                ...
                <tbody>
                    @php
                        $totalDays = \Carbon\Carbon::parse($program->start_date)->diffInDays(\Carbon\Carbon::parse($program->end_date)) + 1;
                    @endphp
                    ...
                    <tr>
                        ...
                        {{-- MULTI-DAY PROGRESS STATUS --}}
                        <td>
                            @if($enrollment->completion_status == 'completed')
                                <span class="status-badge completed">Completed ({{ $enrollment->scan_count ?? 0 }}/{{ $totalDays }} Days)</span>
                            @elseif($enrollment->completion_status == 'failed')
                                <span class="status-badge failed">Failed / Absent</span>
                            @else
                                <span class="status-badge ongoing" style="background: #f1f5f9; color: #475569; border-color: #e2e8f0;">Enrolled ({{ $enrollment->scan_count ?? 0 }}/{{ $totalDays }} Days)</span>
                            @endif
                        </td>
