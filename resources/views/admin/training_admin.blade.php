...
<body>
  ...
  <div class="container">
    ...
    <main>
      ...
      {{-- PENDING SUPERVISOR REQUESTS (Only visible if there are requests) --}}
      @if(isset($pendingRequests) && $pendingRequests->count() > 0)
      <div class="section-container" style="border-top: 4px solid #f59e0b; padding: 0; overflow: hidden; margin-bottom: 30px;">
          <div style="padding: 20px 25px; background: #fffbeb; border-bottom: 1px solid #fde68a;">
              <h3 class="section-title" style="color: #b45309;"><i class="fa-solid fa-code-pull-request"></i> Pending Training Requests</h3>
              <p style="margin: 5px 0 0 0; font-size: 13px; color: #92400e;">Supervisors have requested the following training programs. Review and approve to add them to the calendar.</p>
          </div>
          ...
          <table class="hr-table">
              ...
              <tbody>
                  ...
                  <tr>
                      <td>
                          {{-- Shows the Supervisor who requested it --}}
                          <div style="font-weight:600; color:#0f172a;"><i class="fa-solid fa-user-tie" style="color: #64748b;"></i> {{ $req->requester->name ?? 'Unknown Supervisor' }}</div>
                          
                          {{-- Shows the Target Department --}}
                          <div style="font-size: 13px; color:#2563eb; font-weight: 500; margin-top: 3px;">
                              <i class="fa-solid fa-building"></i> {{ $req->department->department_name ?? 'General (All)' }} Dept
                          </div>
                          
                          <div style="font-size: 11px; color:#94a3b8; margin-top: 4px;">Submitted {{ $req->created_at->format('M d, Y') }}</div>
                      </td>
                      <td>
                          <div style="font-weight:600; color:#2563eb; font-size: 15px;">{{ $req->training_name }}</div>
                          <div style="font-size: 13px; color:#475569; margin-top: 3px;">
                              <i class="fa-regular fa-calendar"></i> {{ \Carbon\Carbon::parse($req->start_date)->format('M d, Y') }} | <i class="fa-solid fa-laptop"></i> {{ $req->mode }}
                          </div>
                      </td>
                      <td style="max-width: 300px;">
                          <div style="font-size: 13px; color:#334155; margin-bottom: 5px;"><strong>Budget:</strong> RM {{ number_format($req->budget ?? 0, 2) }}</div>
                          <div style="font-size: 13px; color:#64748b; font-style: italic; white-space: normal;">"{{ Str::limit($req->purpose, 80) }}"</div>
                      </td>
                      <td style="text-align:right; white-space: nowrap;">
                          <div style="display: flex; gap: 8px; justify-content: flex-end;">
                              <form action="{{ route('admin.training.approve_request', $req->training_id) }}" method="POST" style="margin: 0;">
                                  @csrf
                                  <button type="submit" class="btn-icon" style="background: #10b981; color: white; border: none; padding: 8px 15px; border-radius: 6px; font-weight: 600;" title="Approve Request">Approve</button>
                              </form>
                              <form action="{{ route('admin.training.reject_request', $req->training_id) }}" method="POST" style="margin: 0;" onsubmit="return confirm('Reject this training request?')">
                                  @csrf
                                  <button type="submit" class="btn-icon delete" style="padding: 8px 15px; border-radius: 6px; font-weight: 600;">Reject</button>
                              </form>
                          </div>
                      </td>
                  </tr>
        ...
        <form id="trainingForm" action="{{ route('admin.training.store') }}" method="POST">
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="date" id="startDate" name="startDate" min="{{ \Carbon\Carbon::today()->format('Y-m-d') }}" required>
                </div>
                <div class="form-group">
  ...
  <script>
    ...
    // UI Validation: Date & Time logic constraints
    ...
    function enforceTimeConstraints() {
        ...
        // Get strict local time in HH:MM format
        const now = new Date();
        const currentHours = String(now.getHours()).padStart(2, '0');
        const currentMinutes = String(now.getMinutes()).padStart(2, '0');
        const timeNow = `${currentHours}:${currentMinutes}`;

        // 1. Lock End Date based on Start Date
        ...
        // 2. If Start Date is TODAY, Start Time cannot be in the past
        if (startDateInput.value === today) {
            startTimeInput.min = timeNow;
            if (startTimeInput.value && startTimeInput.value < timeNow) {
        ...
        // 3. If Start Date and End Date are the SAME, End Time must be after Start Time
    ...
    startDateInput.addEventListener('change', enforceTimeConstraints);
    endDateInput.addEventListener('change', enforceTimeConstraints);
    startTimeInput.addEventListener('change', enforceTimeConstraints);
    ...
    function openAddModal() {
        ...
        // NEW: Lock Start Date to strictly TODAY (Server Local Time)
        const today = "{{ \Carbon\Carbon::today()->format('Y-m-d') }}";
    ...
    function openEditModal(data) {
        ...
        // Remove the min date restriction when editing historical records
        ...
        if(data.end_time) {
            document.getElementById('endTime').value = data.end_time.substring(0, 5); 
        } else {
