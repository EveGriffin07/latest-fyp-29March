...
{{-- QUICK ACTIONS --}}
                    <div style="margin-top: 30px; text-align: left;">
                        <span class="info-label" style="margin-bottom: 10px;">Pre-Interview Actions</span>
                        ...
                        {{-- The Hire and Reject buttons have been moved to the Evaluation Form! --}}
                    </div>
                ...
                <div class="content-card">
                    ...
                    @if($application->interviewer_status === 'Evaluated')
                    <div class="info-card" style="border-top: 4px solid #8b5cf6; margin-bottom: 25px;">
                        <h3 class="section-title" style="border: none; margin: 0; margin-bottom: 15px;"><i class="fa-solid fa-user-tie" style="color: #8b5cf6;"></i> Technical Interviewer Feedback</h3>
                        
                        <div style="display: flex; gap: 20px; margin-bottom: 15px; background: #f8fafc; padding: 15px; border-radius: 8px; border: 1px solid #e2e8f0;">
                            <div style="flex: 1;">
                                <span class="info-label" style="font-size: 12px; color: #64748b; font-weight: 600;">Interviewer</span>
                                <div style="font-weight: 600; color: #0f172a;">{{ $application->interviewer->name ?? 'Unknown' }}</div>
                            </div>
                            <div style="flex: 1;">
                                <span class="info-label" style="font-size: 12px; color: #64748b; font-weight: 600;">Technical Score</span>
                                <div style="font-weight: 700; color: #2563eb; font-size: 16px;">{{ $application->supervisor_score }} / 100</div>
                            </div>
                            <div style="flex: 1;">
                                <span class="info-label" style="font-size: 12px; color: #64748b; font-weight: 600;">Recommendation</span>
                                @if($application->supervisor_recommendation == 'Hire')
                                    <div style="font-weight: 700; color: #166534;"><i class="fa-solid fa-check"></i> Hire</div>
                                @elseif($application->supervisor_recommendation == 'Reject')
                                    <div style="font-weight: 700; color: #dc2626;"><i class="fa-solid fa-xmark"></i> Reject</div>
                                @else
                                    <div style="font-weight: 700; color: #b45309;"><i class="fa-solid fa-clock"></i> Shortlist (KIV)</div>
                                @endif
                            </div>
                        </div>

                        <div>
                            <span class="info-label" style="font-size: 12px; color: #64748b; font-weight: 600;">Interviewer Notes</span>
                            <p style="font-size: 14px; color: #334155; line-height: 1.6; margin: 0; white-space: pre-line;">"{{ $application->supervisor_notes }}"</p>
                        </div>
                    </div>
                    @endif
                            ...
                            {{-- NEW: FINAL DECISION DROPDOWN --}}
                            <div style="margin-top: 15px; padding-top: 15px; border-top: 1px dashed #cbd5e1;">
                                <label class="info-label">Final Decision <span style="color:#dc2626">*</span></label>
                                <select name="status" class="eval-input" required {{ $isLocked }} style="{{ $lockedBg }}; font-weight: 500;">
                                    {{-- Safe default if no decision is made yet --}}
                                    <option value="Interview" {{ $application->app_stage == 'Interview' ? 'selected' : '' }}>No decision yet (Keep in Interview Stage)</option>
                                    
                                    {{-- Final Actions --}}
                                    <option value="Hired" {{ $application->app_stage == 'Hired' ? 'selected' : '' }} style="color: #166534; font-weight: 600;">Hire Candidate</option>
                                    <option value="Rejected" {{ $application->app_stage == 'Rejected' ? 'selected' : '' }} style="color: #dc2626; font-weight: 600;">Reject Candidate</option>
                                </select>
                            </div>

                            <div style="text-align: right; margin-top: 25px;">
                                <button type="submit" class="btn-action" style="background: {{ is_null($application->interview_datetime) ? '#94a3b8' : '#2563eb' }}; width: 100%; padding: 14px 30px; display: flex; justify-content: center; font-size: 15px; {{ $lockedBg }}" {{ $isLocked }}>
                                    <i class="fa-solid fa-floppy-disk"></i> Save Evaluation & Apply Decision
                                </button>
                            </div>
                        </form>
                    </div>
        ...
        <div class="hrms-modal-box">
            ...
            <form action="{{ route('admin.applicants.schedule', $application->application_id) }}" method="POST">
                ...
                <div style="margin-bottom: 15px;">
                    <label style="font-size: 13px; font-weight: 600; color: #334155;">Assign Interviewer (Manager/Supervisor) <span style="color:#dc2626">*</span></label>
                    <select name="interviewer_id" class="hrms-input" required>
                        <option value="" disabled selected>Select an Interviewer...</option>
                        @foreach($managers as $manager)
                            <option value="{{ $manager->user_id }}" {{ $application->interviewer_id == $manager->user_id ? 'selected' : '' }}>
                ...
                <div style="margin-bottom: 15px;">
                    <label style="font-size: 13px; font-weight: 600; color: #334155;">Interview Date & Time <span style="color:#dc2626">*</span></label>
                ...
                <div style="margin-bottom: 25px;">
                    <label style="font-size: 13px; font-weight: 600; color: #334155;">Location or Meeting Link <span style="color:#dc2626">*</span></label>
                ...
                {{-- Show current interviewer status if one is already assigned --}}
                @if($application->interviewer_id)
                    <div style="margin-bottom: 20px; padding: 10px; background: #f8fafc; border-radius: 6px; border: 1px solid #e2e8f0; font-size: 13px;">
                        <strong>Current Status:</strong> 
                        <span style="color: {{ $application->interviewer_status == 'Accepted' ? '#166534' : ($application->interviewer_status == 'Rejected' ? '#dc2626' : '#b45309') }}; font-weight: 600;">
                            {{ $application->interviewer_status }}
                        </span>
                        @if($application->interviewer_remarks)
                            <br><span style="color: #64748b; font-style: italic;">"{{ $application->interviewer_remarks }}"</span>
                        @endif
                    </div>
                @endif
                ...
                <div style="text-align: right;">
                    ...
                    <button type="submit" style="background: #2563eb; color: white; border: none; padding: 10px 20px; border-radius: 6px; font-weight: 600; cursor: pointer;">Send Invite</button>
                </div>
            </form>
        </div>
    </div>
