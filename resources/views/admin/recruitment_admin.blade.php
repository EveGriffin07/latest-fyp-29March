...
<head>
  ...
  <style>
      ...
      /* PAGINATION STYLES */
      nav .pagination { display: flex; list-style: none; padding: 0; margin: 0; gap: 8px; justify-content: center; align-items: center; }
      ...
      nav .page-item .page-link:hover { background: #f1f5f9; color: #0f172a; border-color: #94a3b8; }
      nav .page-item.active .page-link { background: #2563eb; color: #fff; border-color: #2563eb; }
      nav .page-item.disabled .page-link { background: #f8fafc; color: #94a3b8; border-color: #e2e8f0; cursor: not-allowed; }
      
      /* Hides the default "Showing 1 to 10 of 50 results" text if it appears unstyled */
      nav .text-sm.text-gray-700 { display: none; }
      /* MODAL STYLES */
...
<body>
  ...
  <div class="container">
    ...
    <main>
      ...
      {{-- NEW: FILTER FORM --}}
      <form method="GET" action="{{ route('admin.recruitment.index') }}" style="display: flex; gap: 15px; margin-bottom: 20px; align-items: flex-end; background: #fff; padding: 20px; border-radius: 12px; border: 1px solid #e5e7eb; box-shadow: 0 4px 15px rgba(0,0,0,0.03);">
          <div style="flex: 1;">
              <label style="font-size: 13px; font-weight: 600; color: #475569; display: block; margin-bottom: 8px;">Filter by Department</label>
              <select name="department" style="width: 100%; padding: 10px 12px; border-radius: 8px; border: 1px solid #cbd5e1; font-family: inherit; font-size: 14px;">
                  <option value="">All Departments</option>
                  @foreach($departments as $dept)
                      <option value="{{ $dept->department_name }}" {{ request('department') == $dept->department_name ? 'selected' : '' }}>
          ...
          <div style="flex: 1;">
              <label style="font-size: 13px; font-weight: 600; color: #475569; display: block; margin-bottom: 8px;">Filter by Status</label>
              <select name="status" style="width: 100%; padding: 10px 12px; border-radius: 8px; border: 1px solid #cbd5e1; font-family: inherit; font-size: 14px;">
          ...
          <div style="display: flex; gap: 10px;">
              ...
              <a href="{{ route('admin.recruitment.index') }}" style="background: #f1f5f9; color: #475569; border: 1px solid #cbd5e1; padding: 10px 20px; border-radius: 8px; font-weight: 600; font-size: 14px; text-decoration: none; display: inline-flex; align-items: center; transition: 0.2s;">
                  Clear
              </a>
          </div>
      </form>
      ...
      <div style="margin-top: 20px;">
          {{-- appends(request()->query()) ensures filters stay in the URL when changing pages --}}
          {{ $jobPosts->appends(request()->query())->links('pagination::bootstrap-4') }}
      </div>
