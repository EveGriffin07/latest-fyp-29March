...
<body>
  ...
  <div class="container dashboard-shell">
    ...
    <main class="content">
            ...
            <div class="training-card">
                ...
                <div class="t-body">
                    ...
                    <div class="t-info"><i class="fa-regular fa-clock"></i> 
                        ...
                        @if($enrollment->training->end_time)
                            - {{ \Carbon\Carbon::parse($enrollment->training->end_time)->format('h:i A') }}
                        @endif
                    </div>
                </div>
      ...
      {{-- ======================================================== --}}
      {{-- NEW SECTION: AVAILABLE TRAINING CATALOG (SELF-ENROLLMENT) --}}
      {{-- ======================================================== --}}
      <h3 class="section-title" style="margin-top: 40px;"><i class="fa-solid fa-compass" style="color: #8b5cf6;"></i> Explore Available Courses</h3>
      <p style="color: #64748b; font-size: 14px; margin-top: -15px; margin-bottom: 20px;">Browse and self-enroll in company-approved upskilling programs.</p>
      
      @if(isset($availableTrainings) && $availableTrainings->count() > 0)
        <div class="training-grid">
            @foreach($availableTrainings as $training)
            <div class="training-card" style="border-top: 4px solid #8b5cf6;">
                <div class="t-header">
                    <span class="t-badge {{ $training->mode == 'Online' ? 'badge-online' : 'badge-onsite' }}">
                        {{ $training->mode }}
                    </span>
                    <span style="font-size:13px; color:#475569; font-weight:600;">
                ...
                <div class="t-body">
                    <span class="t-title">{{ $training->training_name }}</span>
                    <div class="t-info"><i class="fa-solid fa-user-tie"></i> {{ $training->provider }}</div>
                    
                    @if($training->mode == 'Online')
                        <div class="t-info"><i class="fa-solid fa-video"></i> Virtual Session</div>
                    @else
                        <div class="t-info"><i class="fa-solid fa-location-dot"></i> {{ Str::limit($training->location, 30) }}</div>
                    @endif
                    
                    <div class="t-info"><i class="fa-regular fa-clock"></i> 
                        {{ $training->start_time ? \Carbon\Carbon::parse($training->start_time)->format('h:i A') : 'Time TBD' }}
                        @if($training->end_time)
                            - {{ \Carbon\Carbon::parse($training->end_time)->format('h:i A') }}
                        @endif
                    </div>
                </div>
                
                <div class="t-footer" style="text-align: center;">
                    <a href="{{ route('employee.training.show', $training->training_id) }}" style="width: 100%; display: inline-flex; align-items: center; justify-content: center; gap: 8px; padding: 10px; background: #f8fafc; border: 1px solid #cbd5e1; color: #0f172a; border-radius: 6px; font-weight: 600; text-decoration: none; transition: 0.2s;">
                        <i class="fa-solid fa-eye" style="color: #8b5cf6;"></i> View Details
                    </a>
                </div>
            </div>
            @endforeach
      ...
      @else
        <div class="empty-state" style="margin-bottom: 40px; background: #f8fafc;">
            <i class="fa-solid fa-book-open-reader" style="font-size: 32px; margin-bottom: 15px; color: #cbd5e1;"></i><br>
            <strong style="color: #334155;">No new courses available.</strong><br>
            You are currently enrolled in all available active training programs.
        </div>
      @endif
