   ...
   public function index(Request $request)
    {
        ...
        // This now holds the Department name they selected
        ...
        if ($industryInterest) {
            $recommendedJobs = JobPost::where('job_status', 'Open')
                ...
                ->where(function($query) use ($industryInterest) {
                    // Match the Department exact name, or fallback to checking the job title
                    $query->where('department', $industryInterest)
                          ->orWhere('job_title', 'LIKE', "%{$industryInterest}%");
                })
                ->latest()
                ->take(4) // Only show the 4 newest matches
                ->get();
        }
        ...
        // Paginate (8 per page) and keep the search query in the URL links
        $allJobs = $query->latest()->paginate(8)->appends($request->all());
    ...
    public function profile()
    {
        ...
        // Eager load everything, including the new certifications!
        $profile = ApplicantProfile::with(['educations', 'languages', 'skills', 'experiences', 'certifications'])
            ->firstOrCreate(
                ['user_id' => $user->user_id],
                ['full_name' => $user->name, 'email' => $user->email]
            );

        // Fetch all departments for the frontend autocomplete dropdown
        $departments = Department::orderBy('department_name')->get();

        return view('applicant.profile', compact('user', 'profile', 'departments'));
    }
    ...
    public function deleteCertification($id)
    {
        ...
        // Ensure the certification actually belongs to this user before deleting
        $certification = $profile->certifications()->findOrFail($id);
