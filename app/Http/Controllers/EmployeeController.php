...
class EmployeeController extends Controller
{
    public function index()
    {
        ...
        // 6. Fetch Latest Announcements (Richard's Logic - Department Filtered)
        $announcements = Announcement::where('publish_at', '<=', now())
            ->where(function ($query) use ($employee) {
                // 1. Show if the announcement is for Everyone/All
                $query->whereIn('audience_type', ['All', 'Everyone', 'Company', 'General', 'All Employees'])
                      ->orWhereNull('audience_type')
                      // 2. OR show if it matches this employee's department ID
        ...
        // 8. For supervisors: leave requests pending their approval
        ...
        // ========================================================
        // 9. NEW: Fetch Upcoming Interviews for Supervisors/Managers (Richard's Logic)
        // ========================================================
        $upcomingInterviews = collect(); 
        $role = strtolower(Auth::user()->role ?? '');
        
        if (in_array($role, ['supervisor', 'manager'])) {
            $upcomingInterviews = \App\Models\Application::with(['applicant', 'job'])
                ->where('interviewer_id', Auth::id())
                ->where('app_stage', 'Interview')
                ->whereIn('interviewer_status', ['Pending', 'Accepted']) 
    ...
    private function buildOvertimeReport($user, $employee, ?Carbon $periodStart = null, ?Carbon $periodEnd = null): array
    {
        ...
        // Pre-fill 12 months so the chart doesn't skip months with 0 overtime
