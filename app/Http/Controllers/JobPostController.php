...
class JobPostController extends Controller
{
    ...
    // 1. List all active jobs & Pending Requisitions (With Filters & Pagination)
    public function index(Request $request) // <-- Added Request $request here
    {
        ...
        // 1. Initialize the JobPost query
        $query = JobPost::query();

        // 2. Apply Department Filter if user selected one
        if ($request->filled('department')) {
            $query->where('department', $request->department);
        }

        // 3. Apply Status Filter if user selected one
        if ($request->filled('status')) {
            $query->where('job_status', $request->status);
        }

        // 4. Get Paginated Results (10 per page) instead of all at once
        $jobPosts = $query->latest()->paginate(6); 
        ...
        // Fetch departments for the Add/Edit Modal AND the Filter Dropdown
        // Using orderBy makes the dropdown alphabetical and easier to use!
